...
def organize_files(source_dir, dry_run=False, in_place=False, min_size_mb=0, date_prefixing=None, deduping=False, delete_duplicates=False, fallback_mode='skip'):
    ...
    for item in source_path.iterdir():
        ...
        if matching_rule:
            ...
            if 'rename_prefix' in rule_action:
                ...
                final_file_name = f"{prefix}{final_file_name}"
                stats.increment_count('files_renamed')

            rule_action_delete = rule_action.get('delete_file', False)

        if target_folder_name is None and not rule_action_delete:
            if fallback_mode == 'skip':
                logging.info(f"Skipping {item.name}: No move instruction from custom rule.")
                stats.increment_count('files_skipped')
                continue
            elif fallback_mode == 'default':
                target_folder_name, final_file_name, _ = _get_target_names(item, date_prefixing, FILE_TYPE_MAP)
                logging.info(f"Using default rule for {item.name}. Target folder: {target_folder_name}")
            elif fallback_mode == 'prompt':
                default_target = FILE_TYPE_MAP.get(item.suffix.lower().lstrip('.'), 'Miscellaneous')
                if not dry_run:
                    prompt_msg = f"No custom rule matched for '{item.name}'. Use default rule ('{default_target}')? (Y/N/Abort): "
                    user_choice = input(prompt_msg).upper()
                    if user_choice == 'Y':
                        target_folder_name, final_file_name, _ = _get_target_names(item, date_prefixing, FILE_TYPE_MAP)
                        logging.info(f"User approved default rule for {item.name}. Target folder: {target_folder_name}")
                    elif user_choice == 'N':
                        logging.info(f"User chose to skip {item.name}.")
                        stats.increment_count('files_skipped')
                        continue
                    else:
                        print("Aborting organization based on user choice.")
                        sys.exit(0)
                else:
                    logging.info(f"[DRY-RUN] Would prompt for {item.name}. Default target is: {default_target}. Simulating skip (no action in dry-run).")
                    stats.increment_count('files_skipped')
                    continue

        if target_folder_name is None and not rule_action_delete:
             logging.info(f"Skipping {item.name}: No move destination explicitly determined.")
             stats.increment_count('files_skipped')
             continue
...
if __name__ == "__main__":
    ...
    organize_files(args.source_dir, args.dry_run, args.in_place, args.min_size_mb,
                   args.date_prefixing, args.deduping, args.delete_duplicates, args.fallback_mode)
